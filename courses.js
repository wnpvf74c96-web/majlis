...
const KEY='majlis.courses.v1';
const ACTIVE='majlis.courses.active';
const DB_NAME='majlis-courses-db';
const STORE='files';
const loadCourses=()=>{try{const v=JSON.parse(localStorage.getItem(KEY)||'[]');return Array.isArray(v)?v:[]}catch{return[]}};
const saveCourses=v=>{localStorage.setItem(KEY,JSON.stringify(v));window.dispatchEvent(new CustomEvent('majlis:courses-changed'))};
const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
const id=()=>`${Date.now().toString(36)}${Math.random().toString(36).slice(2,8)}`;
let courses=loadCourses(),activeId=localStorage.getItem(ACTIVE)||'',filter='all',query='';
function openDB(){return new Promise((resolve,reject)=>{const req=indexedDB.open(DB_NAME,1);req.onupgradeneeded=()=>{const db=req.result;if(!db.objectStoreNames.contains(STORE)){const s=db.createObjectStore(STORE,{keyPath:'id'});s.createIndex('courseId','courseId')}};req.onsuccess=()=>resolve(req.result);req.onerror=()=>reject(req.error)})}
async function filePut(rec){const db=await openDB();return new Promise((res,rej)=>{const tx=db.transaction(STORE,'readwrite');tx.objectStore(STORE).put(rec);tx.oncomplete=()=>res();tx.onerror=()=>rej(tx.error)})}
async function fileGet(fileId){const db=await openDB();return new Promise((res,rej)=>{const r=db.transaction(STORE).objectStore(STORE).get(fileId);r.onsuccess=()=>res(r.result||null);r.onerror=()=>rej(r.error)})}
async function filesFor(courseId){const db=await openDB();return new Promise((res,rej)=>{const store=db.transaction(STORE).objectStore(STORE);const idx=store.index('courseId');const r=idx.getAll(courseId);r.onsuccess=()=>res(r.result||[]);r.onerror=()=>rej(r.error)})}
...
<div id="courseDetail" class="course-detail" hidden>
  <div class="course-detail-head"><div><div class="label">COURS OUVERT</div><h3 id="courseDetailTitle">Matière</h3><div id="courseDetailSub" class="muted"></div></div><div class="course-detail-actions"><button id="courseStudy" class="primary small" type="button">Étudier avec Majlis</button><button id="courseBack" class="ghost small" type="button">Fermer</button><button id="courseDelete" class="ghost small course-danger" type="button">Supprimer</button></div></div>
...
function bind(){
 $('courseNew').onclick=()=>{const f=$('courseCreate');f.hidden=!f.hidden;if(!f.hidden)setTimeout(()=>$('courseNewName')?.focus(),50)};
 $('courseCreateBtn').onclick=createCourse;
 $('courseNewName').onkeydown=e=>{if(e.key==='Enter'){e.preventDefault();createCourse()}};
 $('courseSearch').oninput=e=>{query=e.target.value.trim().toLowerCase();renderGrid()};
 $('courseFilter').onchange=e=>{filter=e.target.value;renderGrid()};
 $('courseBack').onclick=()=>{activeId='';localStorage.removeItem(ACTIVE);$('courseDetail').hidden=true;renderGrid()};
 $('courseDelete').onclick=removeCourse;
 $('courseStudy').onclick=studyCourse;
...
function createCourse(){const name=$('courseNewName').value.trim();if(!name)return;const c={id:id(),name,code:$('courseNewCode').value.trim(),semester:$('courseNewSemester').value,exam:$('courseNewExam').value||'',progress:0,notes:'',created:new Date().toISOString(),updated:new Date().toISOString()};courses.unshift(c);saveCourses(courses);['courseNewName','courseNewCode','courseNewExam'].forEach(k=>$(k).value='');$('courseCreate').hidden=true;openCourse(c.id)}
...
window.MajlisCourses={open:cid=>{window.MajlisBoard?.show?.('courses');if(cid)setTimeout(()=>openCourse(cid),120)},refresh:render};document.readyState==='loading'?document.addEventListener('DOMContentLoaded',init):init();
})();